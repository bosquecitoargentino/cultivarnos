...
// ---------------------------------------------------------------------
// Arrastrar para reordenar (dentro de "Personalizar inicio"). La
// implementación real (Pointer Events, handle "≡", placeholder,
// auto-scroll, animación FLIP) vive en motor-lista-reordenable.js —
// compartida con "Ordenar cultivos" (ver abrirOrdenCultivos en
// views/cultivos.js), así toda la app usa un único mecanismo de
// "arrastrar con handle dentro de un modal" en vez de mantener dos
// implementaciones del mismo gesto. Acá solo se traduce el resultado
// (ids en el nuevo orden) a la forma que entiende motor-home-layout.js.
// ---------------------------------------------------------------------

function habilitarArrastreHomeLayout(sheet, onSoltar) {
  const listEl = sheet.querySelector('#home-layout-list');
  habilitarArrastreListaReordenable(sheet, listEl, {
